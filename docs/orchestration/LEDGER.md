...
- Autonomous execution boundary: C0-C13 are complete. The user resumed the frozen C14 prelude on 2026-07-19 after conservative cache cleanup; C14 is active, C15 remains closed and terminal for this run, and C16 is not authorised.
...
- Guardrail: task-attached 20-minute heartbeat `interior-design-c14-orchestration-guardrail` (display name `Interior Design C14-C16 orchestration guardrail`) is active. It may resume only the current verified checkpoint, cannot open a later checkpoint early and explicitly prohibits Blender on this Mac.
