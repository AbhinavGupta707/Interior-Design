...
The local app was composed with `C12_OPTION_EVIDENCE_CLASSIFICATION=synthetic-fixture` and the deterministic mock C12 service. Separately connected Chrome completed local fixture sign-in and opened the design-option workspace. The distinct Codex in-app Browser attempt is recorded as unavailable in the root closure below.
...
| Serialized C12 API suite with local PostgreSQL         | PASS: 12 files / 46 tests                                                                                                              | Live local PostgreSQL plus unit/contract                                                                     |
| Clean C1–C12 database migration and production journey | PASS: 1 test from an empty disposable database                                                                                         | Production Next server/browser/BFF → listening API → built spatial-worker executable → PostgreSQL → C5 → C10 |
...
The clean production test created and accepted a real C11 brief, pinned its exact content hash and a committed existing snapshot, then launched the built production Next server against the listening API. A headless Chromium user session loaded the real C11 workspace, observed the exact accepted-brief/model launch link, followed it to the C12 page, and reported no console error. C12 job creation, reads, and both confirmations then traversed the production Next BFF over HTTP. The test spawned the built `services/spatial-worker/dist/src/index.js` executable with only the C12 environment activation enabled; that real service composition claimed the queued job and ran the deterministic planner and PostgreSQL repository. It published two spatially/materially distinct furnishing/finish/light bundles and confirmed them into separate proposed branches. Same-key API replay returned the retained result. Generation created zero proposed snapshots/branches/commits; confirmation created two sibling branches and did not change the existing profile. One confirmed branch compiled through C10 into a valid embedded glTF 2.0 GLB (7,164 bytes, 6 nodes, 18 triangles) with a verified SHA-256, no external URI, and exact furnishing/finish/light element IDs retained in node, mesh, material, and punctual-light mappings. Captured Next/API/worker-process logs contained safe lifecycle events but excluded the private brief marker, access token, summary, asset ID, operations, placements, and lease-token content.
