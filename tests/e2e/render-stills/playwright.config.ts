...
const reuseExistingServer = process.env.C14_REUSE_EXISTING_SERVERS === "1";

export default defineConfig({
  expect: { timeout: 25_000 },
  fullyParallel: false,
  outputDir: "/tmp/c14-render-stills-playwright-results",
  projects: [
    {
  ...
  retries: 0,
  testDir: __dirname,
  testMatch: "render-stills.spec.ts",
  timeout: 90_000,
  use: {
  ...
  webServer: [
    {
      command: "node --experimental-strip-types tests/e2e/render-stills/mock-c14-backend.mjs",
      cwd: repositoryRoot,
      reuseExistingServer,
      timeout: 60_000,
      url: "http://127.0.0.1:4353/health",
    },
    {
      command:
        "HOME_DESIGN_API_BASE_URL=http://127.0.0.1:4353 C14_RENDER_EVIDENCE_CLASSIFICATION=synthetic-fixture pnpm --filter @interior-design/web dev --hostname 127.0.0.1 --port 4352",
      cwd: repositoryRoot,
      reuseExistingServer,
      timeout: 60_000,
