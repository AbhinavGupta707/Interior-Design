...
import { PostgresModelOperationRepository } from "./modules/models/operations/postgres.js";
import { registerModelOperationRoutes } from "./modules/models/operations/routes.js";
import { ModelOperationService } from "./modules/models/operations/service.js";
import type {
  ModelOperationClock,
  ModelOperationRepository,
  ModelOperationUuidFactory,
} from "./modules/models/operations/types.js";
import {
...
type C5EnvironmentSource = Readonly<Record<string, string | undefined>>;

export interface C5ModuleOptions {
...
export interface C5Module {
  readonly readinessChecks: readonly ReadinessCheck[];
  readonly service: ModelOperationService;
}

function databaseUrl(
  runtimeEnvironment: RuntimeEnvironment,
  environment: C5EnvironmentSource,
  override: string | undefined,
): string {
  const configured =
    override ??
    environment.C5_DATABASE_URL ??
    environment.C4_DATABASE_URL ??
    environment.C1_DATABASE_URL;
  if (configured !== undefined && configured.length > 0) return configured;
  if (runtimeEnvironment === "production") {
    throw new Error(
      "C5_DATABASE_URL, C4_DATABASE_URL, or C1_DATABASE_URL is required in production.",
    );
  }
...
export function registerC5Module(
  server: FastifyInstance,
  runtimeEnvironment: RuntimeEnvironment,
  environment: C5EnvironmentSource,
  options: C5ModuleOptions = {},
): C5Module {
  const needsDatabase =
  ...
  const repository =
    options.repository ??
    new PostgresModelOperationRepository(sql as Sql, {
    ...
    readinessChecks.push({
      ...
      check: async () => {
        const rows = await sql<{ readonly id: string }[]>`
          ...
          WHERE id = '0005_model_operations'
          LIMIT 1
        `;
        if (rows.length !== 1) throw new Error("C5 database migration is not applied.");
      },
    });
  }
...
async function firstExistingPath(candidates: readonly string[]): Promise<string> {
  ...
  throw new Error("The required C5 migration file could not be located.");
}

async function migrationPath(): Promise<string> {
  return firstExistingPath([
    path.resolve(process.cwd(), "services/platform-api/migrations/0005_model_operations.sql"),
    path.resolve(process.cwd(), "migrations/0005_model_operations.sql"),
  ]);
}

export async function applyC5Migration(sql: Sql, filePath?: string): Promise<void> {
...
async function runAdminCommand(command: string | undefined): Promise<void> {
  ...
  if (command !== "cleanup-previews" && command !== "migrate" && command !== "verify-replay") {
    throw new Error("Expected: migrate, cleanup-previews, or verify-replay.");
  }
  ...
  try {
    ...
    const repository = new PostgresModelOperationRepository(sql);
    if (command === "cleanup-previews") {
      const deletedPreviewCount = await repository.cleanupExpiredPreviews();
      process.stdout.write(`${JSON.stringify({ command, deletedPreviewCount, status: "ok" })}\n`);
      return;
    }
    const verified = await repository.verifyEveryBranch();
    process.stdout.write(
      `${JSON.stringify({ branchCount: verified.length, command, status: "ok" })}\n`,
    );
  } finally {
    await sql.end({ timeout: 5 });
  }
