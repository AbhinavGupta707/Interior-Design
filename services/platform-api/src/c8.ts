...
import { PostgresReconstructionRepository } from "./modules/reconstruction/postgres.js";
import { registerReconstructionRoutes } from "./modules/reconstruction/routes.js";
import { ReconstructionService } from "./modules/reconstruction/service.js";
import type {
  ReconstructionClock,
  ReconstructionRepository,
  ReconstructionTelemetry,
  ReconstructionUuidFactory,
} from "./modules/reconstruction/types.js";
import {
...
type C8EnvironmentSource = Readonly<Record<string, string | undefined>>;

export interface C8ModuleOptions {
...
function databaseUrl(
  ...
  environment: C8EnvironmentSource,
  ...
  if (runtimeEnvironment === "production") {
    throw new Error(
      "C8_DATABASE_URL, C7_DATABASE_URL, C6_DATABASE_URL, C2_DATABASE_URL, or C1_DATABASE_URL is required in production.",
    );
  }
...
export function registerC8Module(
  server: FastifyInstance,
  runtimeEnvironment: RuntimeEnvironment,
  environment: C8EnvironmentSource,
  options: C8ModuleOptions = {},
): C8Module {
  const needsDatabase =
  ...
  const repository =
    options.repository ??
    new PostgresReconstructionRepository(sql as Sql, {
      ...(options.clock === undefined ? {} : { clock: options.clock }),
      ...(options.uuid === undefined ? {} : { uuid: options.uuid }),
    });
  const service = new ReconstructionService(repository, options.telemetry);
  registerReconstructionRoutes(server, identity, projects, service);
    ...
    readinessChecks.push({
      ...
      check: async () => {
        const rows = await sql<{ readonly id: string }[]>`
          ...
          WHERE id = '0008_reconstruction' LIMIT 1
        `;
        if (rows.length !== 1) throw new Error("C8 database migration is not applied.");
      },
    });
  }
...
async function firstExistingPath(candidates: readonly string[]): Promise<string> {
  ...
  throw new Error("The required C8 migration file could not be located.");
}

export async function applyC8Migration(sql: Sql, filePath?: string): Promise<void> {
  const resolvedPath =
    filePath ??
    (await firstExistingPath([
      path.resolve(process.cwd(), "services/platform-api/migrations/0008_reconstruction.sql"),
      path.resolve(process.cwd(), "migrations/0008_reconstruction.sql"),
    ]));
