  ...
  @preconcurrency ARSessionDelegate
{
  ...
  func captureKeyframe(
    ...
    guard tracking != .unavailable else {
      throw C14_8GuidedCaptureEngineError.trackingUnavailable
    }
    ...
    let depth = frame.sceneDepth.map(Self.depthBytes)
  ...
  private static func depthBytes(_ depth: ARDepthData) -> (data: Data, height: Int, width: Int) {
    ...
    for row in 0..<height {
      data.append(base.advanced(by: row * stride).assumingMemoryBound(to: UInt8.self), count: width * 4)
    }
    return (data, height, width)
  }

  private static func micro(_ value: Float) -> Int64 { Int64((Double(value) * 1_000_000).rounded()) }
  private static func nano(_ value: Float) -> Int64 { Int64((Double(value) * 1_000_000_000).rounded()) }
}
  ...
  func captureKeyframe(
    ...
    return C14_8CapturedKeyframe(
      ...
      sample: C14_8LocalCameraSample(
        ...
        timestampMicroseconds: max(0, Int64(Date().timeIntervalSince(captureStartedAt) * 1_000_000)),
        trackingState: .normal,
        translationMicrometres: C14_8Translation(x: 0, y: 1_500_000, z: 0)
      )
    )
  }
}
