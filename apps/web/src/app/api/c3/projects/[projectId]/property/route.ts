...
export async function PUT(request: Request, context: PropertyRouteContext): Promise<NextResponse> {
  ...
  const body = await parseC3Request(request, selectProjectPropertyRequestSchema);
  ...
  return proxyC3Response({
    ...
    idempotencyKey: key,
    method: "PUT",
    path: `/v1/projects/${details.projectId}/property`,
    responseSchema: projectPropertySchema,
  });
}
